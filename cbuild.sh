...
Options:
  ...
  -v <version>       scala version, can be overridden by 'version=2.12.1 ./cbuild.sh' : default latest nightly
...
echo "re-run as:"
echo version=$version ./cbuild.sh ${@}
...
"dbuild-${DBUILDVERSION}/bin/dbuild"  "$dbuild_args" "$DBUILDCONFIG" "${@}" 2>&1 | tee "dbuild-${DBUILDVERSION}/dbuild.out"
STATUS="$?"
